...
chrome.action.onClicked.addListener((tab) => {
  chrome.storage.local.get(['shortsToggle'], (result) => {
    const includeShorts = result.shortsToggle || false;
    chrome.scripting.executeScript({
      ...
      func: (includeShorts) => {
        ...
        function redirectToPlaylist(channelID) {
          try {
            const playlistType = includeShorts ? 'UU' : 'UULF';
            const playlistURL = `https://www.youtube.com/playlist?list=${playlistType}${channelID.substr(2)}`;
